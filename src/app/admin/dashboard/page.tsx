...
function MOEReportGenerator() {
  ...
  // إظهار المعاينة قبل الطباعة تلقائياً - حل احترافي
  useEffect(() => {
    if (printing && !showPreview) {
      // الخطوة 1: إظهار المعاينة
      setShowPreview(true);
    }
  }, [printing, showPreview]);

  // الخطوة 2: انتظار رسم DOM ثم الطباعة
  useEffect(() => {
    if (printing && showPreview) {
      // استخدام requestAnimationFrame للتأكد من اكتمال الرسم
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          // التأكد من وجود العنصر .sheet
          const sheetElement = document.querySelector('.sheet');
          if (sheetElement) {
            // إعطاء وقت إضافي للصور والمحتوى
            setTimeout(() => {
              window.print();
              // إعادة تعيين حالة الطباعة بعد فترة قصيرة
              setTimeout(() => setPrinting(false), 500);
            }, 300);
          } else {
            console.error('Sheet element not found');
            setPrinting(false);
            alert('حدث خطأ في تحميل المعاينة. حاول مرة أخرى.');
          }
        });
      });
    }
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="flex justify-center gap-4 pt-4">
            ...
            <button 
              ...
              disabled={printing}
              className={`px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-bold text-lg hover:shadow-2xl transition-all transform hover:-translate-y-1 flex items-center gap-3 ${printing ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              {printing ? (
                <>
